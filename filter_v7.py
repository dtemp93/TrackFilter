...
class Filter(object):
    ...
    def alpha(self, dt, pstate, meas_rae, cov_est, LLA, sensor_noise, states):
        ...
        with tf.variable_scope('alpha'):
            ...
            rnn_inpa = tf.concat([dt, pre_res_n, cov_diag_n, delta_state, delta_cov, altitude_n, gravity_n, gs_input, sensor_noise], axis=1)
            ...
            with tf.variable_scope('Cell_2/q_cov', reuse=tf.AUTO_REUSE):
                (outa2, state2) = self.source_fwf2(rnn_inpa2, state=state2)

            outa = tf.concat([outa1, outa2], axis=1)

            with tf.variable_scope('Cell_3/q_cov', reuse=tf.AUTO_REUSE):
                (outb1, state3) = self.source_fwf3(outa, state=state3)

            with tf.variable_scope('Cell_4/q_cov', reuse=tf.AUTO_REUSE):
                (outb2, state4) = self.source_fwf4(outa, state=state4)
            ...
            Rtemp = tf.where(R <= sensor_noise[:, 0, tfna], ones*1000, R)
            rd_mult = tf.concat([ones, ones * Rtemp, ones * Rtemp], axis=1)
            rd = (rd_mult * sensor_noise)

            rdist = tfd.MultivariateNormalDiag(loc=None, scale_diag=rd)
            Rt = rdist.covariance()

            nv = out.shape[1].value // 3
            qm0 = FCL(out, self.num_mixtures*3, activation_fn=tf.nn.elu, weights_initializer=tf.initializers.variance_scaling, normalizer_fn=tfc.layers.layer_norm, scope='q_cov/1', reuse=tf.AUTO_REUSE)
            qm1 = FCL(qm0, qm0.shape[1].value, activation_fn=tf.nn.elu, weights_initializer=tf.initializers.variance_scaling, scope='q_cov/2', reuse=tf.AUTO_REUSE)
            qm2 = FCL(qm1, qm0.shape[1].value, activation_fn=tf.nn.elu, weights_initializer=tf.initializers.variance_scaling, scope='q_cov/3', reuse=tf.AUTO_REUSE)

            qmx = FCL(qm2[:, :nv], self.num_mixtures, activation_fn=tf.nn.softmax, weights_initializer=tf.initializers.variance_scaling, scope='q_cov/x', reuse=tf.AUTO_REUSE)
            qmy = FCL(qm2[:, nv:2*nv], self.num_mixtures, activation_fn=tf.nn.softmax, weights_initializer=tf.initializers.variance_scaling, scope='q_cov/y', reuse=tf.AUTO_REUSE)
            qmz = FCL(qm2[:, 2*nv:], self.num_mixtures, activation_fn=tf.nn.softmax, weights_initializer=tf.initializers.variance_scaling, scope='q_cov/z', reuse=tf.AUTO_REUSE)
            # atx = FCL(qm0[:, 3*nv:], self.num_mixtures, activation_fn=tf.nn.softmax, weights_initializer=tf.initializers.variance_scaling, scope='q_cov/t', reuse=tf.AUTO_REUSE)
    ...
    def beta(self, dt, filtered_state, filtered_covariance, meas_uvw, state3):
        ...
        with tf.variable_scope('beta'):
            ...
            rnn_inp1 = FCL(rnn_inp0, inp_width, activation_fn=tf.nn.elu, weights_initializer=tf.initializers.variance_scaling, scope='rnn_inp1', reuse=tf.AUTO_REUSE)
            ...
            rnn_inp4 = FCL(rnn_inp3, inp_width, activation_fn=tf.nn.elu, weights_initializer=tf.initializers.variance_scaling, scope='rnn_inp4', reuse=tf.AUTO_REUSE)

            (cell_output, state3) = self.source_fwf3(rnn_inp4, state=state3)

            all_outputs = tf.concat([pos_res_uvw, cell_output], axis=2)
            out1_state = FCL(all_outputs, self.num_meas, activation_fn=tf.nn.elu, weights_initializer=tf.initializers.variance_scaling, scope='out1_state', reuse=tf.AUTO_REUSE)
            out2_state = FCL(out1_state, self.num_meas, activation_fn=tf.nn.elu, weights_initializer=tf.initializers.variance_scaling, scope='out2_state', reuse=tf.AUTO_REUSE)
            out3_state = FCL(out2_state, self.num_meas, activation_fn=tf.nn.elu, weights_initializer=tf.initializers.variance_scaling, scope='out3_state', reuse=tf.AUTO_REUSE)
            out4_state = FCL(out3_state, self.num_meas, activation_fn=None, weights_initializer=tf.initializers.variance_scaling, scope='out3_state', reuse=tf.AUTO_REUSE)
    ...
    def forward_step_fn(self, params, inputs):
        ...
        with tf.variable_scope('forward_step_fn'):
            ...
            _, _, mu_t0, Sigma_t0, _, state1, state2, state3, state4, _, _, _, _, _, _, _ = params
            ...
            meas_uvw, Qt, At, Rt, Bt, u, gravity, altitude, states = self.alpha(dt, mu_t0, meas_rae, Sigma_t0, LLA, sensor_noise, states)

            # slider = tf.nn.sigmoid(Sigma_t0)
            # slider = 0.7
            # Qt = (1 - slider) * Qt0 + slider * Qt
            ...
            # num_units = 1
            # Q = tf.layers.dense(mu_t, num_units, activation=None, reuse=tf.AUTO_REUSE, name='Q')
            # K = tf.layers.dense(lpmeas, num_units, activation=None, reuse=tf.AUTO_REUSE, name='K')
            # V = tf.layers.dense(lpmeas, num_units, activation=None, reuse=tf.AUTO_REUSE, name='V')
            ...
            return mu_pred, Sigma_pred, mu_t, Sigma_t, meas_uvw, state1, state2, state3, state4, Qt, Rt, At, Bt, S_inv, weight, u
    ...
    def compute_backwards(self, forward_states):
        with tf.variable_scope('Backward'):
            mu_pred, Sigma_pred, mu_filt, Sigma_filt, meas_uvw, state1, state2, state3, state4, Q, R, A, B, S_inv, weights, u = forward_states
    ...
    def refine(self, filtered_state, filtered_covariance, meas_uvw):
        ...
        with tf.variable_scope('refine'):
            ...
            out1_state = FCL(all_outputs, self.num_meas*2, activation_fn=tf.nn.elu, weights_initializer=tf.initializers.variance_scaling, scope='out1_state', reuse=tf.AUTO_REUSE)
            out2_state = FCL(out1_state, self.num_meas*2, activation_fn=tf.nn.elu, weights_initializer=tf.initializers.variance_scaling, scope='out2_state', reuse=tf.AUTO_REUSE)
            out3_state = FCL(out2_state, self.num_meas*2, activation_fn=tf.nn.elu, weights_initializer=tf.initializers.variance_scaling, scope='out3_state', reuse=tf.AUTO_REUSE)
            out4_state = FCL(out3_state, self.num_meas*2, activation_fn=None, weights_initializer=tf.initializers.variance_scaling, scope='out4_state', reuse=tf.AUTO_REUSE)
            ...
            # weights_initializer=tf.initializers.variance_scaling, scope='rnn_out1c', reuse=tf.AUTO_REUSE)
            # out2 = FCL(out1, self.num_state, activation_fn=None, weights_initializer=tf.initializers.variance_scaling, scope='rnn_out2c', reuse=tf.AUTO_REUSE)
            # cov_pred = FCL(out2, self.num_state, activation_fn=tf.nn.softplus, weights_initializer=tf.initializers.variance_scaling, scope='rnn_out3c', reuse=tf.AUTO_REUSE)
    ...
    def get_elbo(self, backward_states, name=''):
        with tf.variable_scope('ELBO' + name):
            ...
            if self.mode == 'training':
                ...
                trans_error_vel = tf.concat([trans_error[:, :, 0, tfna], trans_error[:, :, 4, tfna], trans_error[:, :, 8, tfna]], axis=2)
            ...
            if self.mode == 'training':
                ...
                self.transition_error = tf.truediv(tf.reduce_sum(trans_error_vel * self.seqweightin[:, :-1, tf.newaxis]), num_el2)
            else:
    ...
    def build_model(self):
        ...
        if use_refinement is True:
            # refine_state = self.compute_refine(filter_out[0], filter_out[1], meas_uvw, Q, R)
        ...
        filter_out = [filter_out[0], fixed_cov]
        ...
        with tf.variable_scope("TrainOps"):
            ...
            self.lower_bound = (self.cov_pos_loss + self.cov_vel_loss + tf.sqrt(self.transition_error)) * self.meas_err_use + self.J_loss
            ...
            tf.summary.scalar("Transition_Error", self.transition_error)
    ...
    def train(self, data_rate, max_exp_seq):
        ...
        for epoch in range(int(start_epoch), self.max_epoch):
            ...
            for minibatch_index in range(n_train_batches):
                ...
                x_data, y_data, batch_number, total_batches, ecef_ref, lla_data, meas_list = ds.load(batch_size=self.batch_size_np, constant=self.constant,
                ...
                if randomize_start:
                    start_idx = random.randint(5, 25) * self.data_rate
                    xl = int(x.shape[1])
                    if start_idx > int(0.5*xl):
                        start_idx = 1
                    x = x[:, start_idx:, :]
                    y = y[:, start_idx:, :]
                ...
                for tstep in range(0, windows):
                    ...
                    fd.update({self.measurement[t]: current_x[:, t, :3].reshape(-1, self.num_meas) for t in range(self.max_seq)})
