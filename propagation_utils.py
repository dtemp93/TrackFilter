...
def unscented_kalman_np(batch_size, seqlen, initial_state, initial_covariance, measurement_list, prev_time, dt0, dt1, lla_datar):
    ...
    for q in range(seqlen):
        ...
        # rae2enu[:, 0, 1] = np.squeeze(R * np.cos(A) * np.cos(E), -1)
        # rae2enu[:, 0, 2] = np.squeeze(-R * np.sin(A) * np.sin(E), -1)
        # rae2enu[:, 1, 0] = np.squeeze(np.cos(A)*np.cos(E), -1)
        # rae2enu[:, 1, 1] = np.squeeze(-R * np.sin(A) * np.cos(E), -1)
        # rae2enu[:, 1, 2] = np.squeeze(-R * np.cos(A)*np.sin(E), -1)
        # rae2enu[:, 2, 0] = np.squeeze(np.sin(E), -1)
        # rae2enu[:, 2, 1] = 0
        # rae2enu[:, 2, 2] = np.squeeze(R * np.cos(E), -1)
        # 
        ...
        # try:
        #     qcholr = np.linalg.cholesky(cov_est0)
        # except:
        #     pdb.set_trace()
        #     pass
        ...
        y_enu = np.squeeze(np.matmul(Ti2e, mu_pred_pos_uvw), -1)
        ...
        # Rt[:, 1, 1] = np.ones_like(el)*5e-3
        # Rt[:, 2, 2] = np.ones_like(el)*0.01
        # Rt = np.fill_diagonal(Rt, r_diag)

        # ned_cov = np.matmul(np.matmul(mmat, Rt), trans(mmat))

        # Rt = np.matmul(np.matmul(Ti2e, enu_cov), trans(mmat))
    ...
    Qt, At, Bt, At2 = get_QP_np(dt1[:, np.newaxis, :], om, zm, I_3z, I_4z, zb,
                                dimension=int(num_state / 3),
                                sjix=om * 5 ** 2,
                                sjiy=om * 5 ** 2,
                                sjiz=om * 5 ** 2,
    ...
    return final_state, Ql, converted_meas, pred_state, pred_covariance
