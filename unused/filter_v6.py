...
class Filter(object):
    ...
    def alpha(self, int_time, dt, pstate, meas_rae, cov_est, Q_m1, R_m1, LLA, sensor_onehot, sensor_noise, states):
        ...
        with tf.variable_scope('alpha'):
            ...
            rnn_inpa = tf.concat([dt, pre_res_n, cov_diag_n, pos_part_n, vel_part_n, acc_part_n, jer_part_n, altitude_n, gravity_n, gs_input, acc_drag, A, E, tf.sqrt(tf.matrix_diag_part(Rt_temp_rae))], axis=1)
            ...
            # rd_set = tf.sqrt(tf.matrix_diag_part(Rt_temp_rae))
            # sig_val = FCL(rd_rnn, self.num_meas, activation_fn=tf.nn.sigmoid,
            #               weights_initializer=tf.initializers.truncated_normal(stddev=0.01),
            #               biases_initializer=tf.initializers.constant(-2.0),
            #               scope='fade/r_cov', reuse=tf.AUTO_REUSE)

            # junction = tf.ones_like(sig_val) - sig_val
            # rd = rd_rnn * sig_val + rd_rnn * junction
            ...
            return meas_uvw, Qt, At, Rt, Bt, u, gravity, altitude, ecef_ref, states
    ...
    def forward_step_fn(self, params, inputs):
        ...
        with tf.variable_scope('forward_step_fn'):
            ...
            meas_uvw, Qt, At, Rt, Bt, u, gravity, ecef_ref, altitude, states = self.alpha(int_time, dt, mu_t0, meas_rae,
            ...
            mu_pred = tf.matmul(self.meas_mat, mu_pred[:, :, tfna])
            pos_res = meas_uvw[:, :, tfna] - mu_pred

            uvw_to_enu = uvw2enu_tf(lat, lon)
            enu_to_uvw = tf.transpose(uvw_to_enu, [0, 2, 1])

            y_enu = tf.squeeze(tf.matmul(uvw_to_enu, mu_pred), -1)
            ...
            mu_t = mu_pred[:, :, tfna] + tf.matmul(gain, pos_res)
            mu_t = mu_t[:, :, 0]
    ...
    def get_elbo(self, backward_states, name=''):
        with tf.variable_scope('ELBO' + name):
            ...
            state_error = all_truth - z_smooth
            ...
            # M1P_meas = tf.matmul(emiss_centered[:, :, :, tfna], self.si_list, transpose_a=True)
            # M2P_meas = tf.matmul(M1P_meas, emiss_centered[:, :, :, tfna])
            #
            # self.MDP_meas = tf.squeeze(M2P_meas, -1) / self.num_meas  # nominally 1
            # self.MDPi_meas = tf.sqrt((tf.ones_like(self.MDP_meas, self.vdtype) / self.MDP_meas))
            # self.maha_loss_meas = tf.sqrt(tf.pow((tf.ones_like(self.MDP_meas) - self.MDP_meas), 2)) * self.seqweightin[:, :, tfna]
            # self.maha_loss_meas = tf.truediv(tf.reduce_sum(self.maha_loss_meas), num_el2)

            M1P_pos = tf.matmul(pos_error[:, :, :, tfna], tf.matrix_inverse(cov_pos), transpose_a=True)
            M2P_pos = tf.matmul(M1P_pos, pos_error[:, :, :, tfna])

            self.MDP_pos = tf.squeeze(M2P_pos, -1) / self.num_meas
            self.MDPi_pos = tf.sqrt((tf.ones_like(self.MDP_pos, self.vdtype) / self.MDP_pos))
            self.maha_loss_pos = (tf.ones_like(self.MDP_pos) - self.MDP_pos) * self.seqweightin[:, :, tfna]
            ...
            self.MDP_vel = tf.squeeze(M2P_vel, -1) / self.num_meas
            self.MDPi_vel = tf.sqrt((tf.ones_like(self.MDP_vel, self.vdtype) / self.MDP_vel))
            self.maha_loss_vel = (tf.ones_like(self.MDP_vel) - self.MDP_vel) * self.seqweightin[:, :, tfna]
            ...
            self.error_loss_full = tf.truediv(tf.reduce_sum(tf.negative(train_cov_full.log_prob(self.state_error)) * self.seqweightin), (num_el2 * 144)) + 1e-5
    ...
    def build_model(self, is_training):
        ...
        with tf.variable_scope("TrainOps"):
            ...
            self.vel_scaled = tf.sqrt((self.rmse_vel / self.rmse_pos))
            self.acc_scaled = tf.sqrt((self.rmse_acc / self.rmse_pos))
            self.jer_scaled = tf.sqrt((self.rmse_jer / self.rmse_pos))
            ...
            self.lower_bound = self.meas_err_use * (self.J_loss + self.vel_scaled)

            # if use_refinement is True:
            #     refinement_error = self.meas_error_refined  # * (self.error_loss_pos_refined + self.error_loss_vel_refined + self.entropy_refined)

            opt1 = tfc.opt.MultitaskOptimizerWrapper(tfc.opt.AdamWOptimizer(weight_decay=1e-4, learning_rate=self.learning_rate, name='opt1'))
            gradvars1 = opt1.compute_gradients(self.lower_bound, other_vars, colocate_gradients_with_ops=False)
            ...
            # if use_refinement is True:
            opt2 = tfc.opt.MultitaskOptimizerWrapper(tfc.opt.AdamWOptimizer(weight_decay=1e-4, learning_rate=self.learning_rate, name='opt2'))
            gradvars2 = opt2.compute_gradients(self.meas_err_use * self.rl, r_vars, colocate_gradients_with_ops=False)
            ...
            opt3 = tfc.opt.MultitaskOptimizerWrapper(tfc.opt.AdamWOptimizer(weight_decay=1e-4, learning_rate=self.learning_rate, name='opt3'))
            gradvars3 = opt3.compute_gradients(self.J_loss + self.error_loss_pos + self.error_loss_vel, q_vars, colocate_gradients_with_ops=False)
            ...
            tf.summary.scalar("Total_Loss", self.J_loss)
            ...
            # tf.summary.scalar("MahalanobisLoss_meas", self.maha_loss_meas)
            # tf.summary.scalar("MahalanobisDistance_meas", tf.truediv(tf.reduce_sum(self.MDP_meas * self.seqweightin[:, :, tfna]), self.num_el))
            # tf.summary.scalar("MahalanobisInverse_meas", tf.truediv(tf.reduce_sum(self.MDPi_meas * self.seqweightin[:, :, tfna]), self.num_el))
            ...
            tf.summary.scalar("RMSE_vel", self.vel_scaled + 1e-5)
            tf.summary.scalar("RMSE_acc", self.acc_scaled + 1e-5)
            tf.summary.scalar("RMSE_jer", self.jer_scaled + 1e-5)
    ...
    def train(self, data_rate, max_exp_seq):
        ...
        for epoch in range(int(start_epoch), self.max_epoch):
            ...
            for minibatch_index in range(n_train_batches):
                ...
                self.fader = 0.6 * np.power(0.95, (step / 100)) + 1e-6
                ...
                for tstep in range(0, windows):
                    ...
                    if tstep == 0:
                        current_state_estimate, current_cov_estimate, prev_state_estimate, prev_covariance_estimate, initial_Q, initial_R, converted_meas = \
                            initialize_filter(self.batch_size_np, initial_time, initial_meas, prev_time, prev_x, current_time, lla_datar, sensor_vector2)
                    ...
                    for _ in range(ittt):
                        ...
                        entropy, qt_out, rt_out, at_out, q_loss, state_fwf1, state_fwf2, state_fwf3, new_meas, summary_str = \
