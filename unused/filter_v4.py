...
class Filter(object):
    def __init__(self, sess, trainable_state=False, state_type='GRU', mode='training',
        ...
        self.decay_steps = (45000 / self.batch_size_np) * (3000 / self.max_seq)
    ...
    def alpha(self, int_time, dt, pstate, meas_rae, cov_est, Q_est, R_est, LLA, sensor_onehot, sensor_noise, states):
        ...
        with tf.variable_scope('alpha'):
            ...
            sjy_l = list()
            sjz_l = list()
            sjxl = [0.001, 0.5, 25, 250]
            sjyl = [0.001, 0.5, 25, 250]
            sjzl = [0.001, 0.5, 25, 250]
            ...
            u = tf.concat([acc_drag[:, 0,  tfna]*0, jx_input,
                           acc_drag[:, 1,  tfna]*0, jy_input,
                           acc_drag[:, 2,  tfna]*0, jz_input], axis=1)
    ...
    def forward_step_fn(self, params, inputs):
        ...
        with tf.variable_scope('forward_step_fn'):
            ...
            # mu_pred = propagatef2(mu_t, u, dt, altitude)
    ...
    def build_model(self, is_training):
        ...
        with tf.variable_scope("TrainOps"):
            ...
            opt1 = tfc.opt.MultitaskOptimizerWrapper(tfc.opt.AdamWOptimizer(weight_decay=1e-3, learning_rate=self.learning_rate, name='opt1'))
            ...
            # if use_refinement is True:
            opt2 = tfc.opt.MultitaskOptimizerWrapper(tfc.opt.AdamWOptimizer(weight_decay=1e-3, learning_rate=self.learning_rate, name='opt2'))
            ...
            opt3 = tfc.opt.MultitaskOptimizerWrapper(tfc.opt.AdamWOptimizer(weight_decay=1e-3, learning_rate=self.learning_rate, name='opt3'))
            gradvars3 = opt3.compute_gradients(((tf.log(self.meas_err_use + 1) + 0.4) * (self.error_loss_pos + self.error_loss_vel)), q_vars, colocate_gradients_with_ops=False)
            ...
            tf.summary.scalar("RMSE_pos", tf.sqrt(self.rmse_pos + 1e-5))
            tf.summary.scalar("RMSE_vel", tf.sqrt(self.rmse_vel + 1e-5))
            tf.summary.scalar("RMSE_acc", tf.sqrt(self.rmse_acc + 1e-5))
            tf.summary.scalar("RMSE_jer", tf.sqrt(self.rmse_jer + 1e-5))
    ...
    def train(self, data_rate, max_exp_seq):
        ...
        for epoch in range(int(start_epoch), self.max_epoch):
            ...
            for minibatch_index in range(n_train_batches):
                ...
                self.fader = 0.9 * np.power(0.95, (step / 250)) + 0.05
    ...
    def test(self, data_rate, max_exp_seq):
        ...
        for minibatch_index in range(n_train_batches):
            ...
            for step in range(0, time_data.shape[1]):
                ...
                # prev_cov = np.concatenate([prev_covariance_estimate[:, -1, np.newaxis, :, :], q_out_t], axis=1)
                # prev_cov = prev_cov[:, idx, np.newaxis, :, :]
