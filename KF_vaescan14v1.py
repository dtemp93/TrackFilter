...
class Filter(object):
    ...
    def forward_step_fn(self, params, inputs):
        ...
        pos_res_uvw = meas_uvw[:, :, tf.newaxis] - tf.matmul(self.meas_mat, mu_pred[:, :, tf.newaxis])

        # r = sqrt(enu(1) * enu(1) + enu(2) * enu(2) + enu(3) * enu(3));
        # az = atan2(enu(1), enu(2));
        # el = asin(enu(3) / r);

    # r_state = tf.norm(mu_pred, axis=1)
    #     az_state = tf.atan()
    ...
    def build_model(self, is_training):
        ...
        with tf.variable_scope("TrainOps"):
            ...
            lower_bound = self.rl * 1 + self.entropy * 0 + self.maha_loss * 0 + self.error_loss_full * 1 + self.rmse_pos * 1 + self.error_loss_pos * 0 + self.meas_error * 0
