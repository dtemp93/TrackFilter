...
class Filter(object):
    ...
    def alpha(self, current_time, int_time, dt, pstate, meas_rae, cov_est, Q_est, R_est, LLA, sensor_onehot, state1=None, state2=None, state3=None):
        ...
        alpha_sr = FCL(rm1[:, 0, tf.newaxis], self.num_mixtures, activation_fn=tf.nn.softmax, scope='r_cov/alphar', reuse=tf.AUTO_REUSE)
        alpha_sa = FCL(rm1[:, 1, tf.newaxis], self.num_mixtures, activation_fn=tf.nn.softmax, scope='r_cov/alphaa', reuse=tf.AUTO_REUSE)
        alpha_se = FCL(rm1[:, 2, tf.newaxis], self.num_mixtures, activation_fn=tf.nn.softmax, scope='r_cov/alphae', reuse=tf.AUTO_REUSE)
        ...
        sr = tf.matmul(alpha_sr, tf.sqrt(sr_vals))
        sa = tf.matmul(alpha_sa, tf.sqrt(sa_vals))
        se = tf.matmul(alpha_se, tf.sqrt(se_vals))
        ...
        # all_alphas = tf.concat([alpha_sr, slpha_sa, alpha_se, qmx, qmy, qmz], axis=1)
    ...
    def build_model(self, is_training):
        ...
        self.learning_rate = tf.train.exponential_decay(self.learning_rate_inp, global_step=self.global_step, decay_steps=10000, decay_rate=0.95, staircase=True)
        ...
        with tf.variable_scope("TrainOps"):
            ...
            lower_bound = self.rl * 1 + self.entropy * 1 + self.maha_loss * 0 + self.error_loss_full * 1 + self.rmse_pos * 1 + self.error_loss_pos * 0 + self.meas_error * 1 + self.error_loss_Q * 0
    ...
    def train(self, data_rate, max_exp_seq):
        ...
        for epoch in range(int(start_epoch), self.max_epoch):
            ...
            for minibatch_index in range(n_train_batches):
                ...
                if e % 5 == 0 and minibatch_index != 0:
    ...
    def evaluate(self, x_train, y_train, ecef_ref, lla_datar, epoch, minibatch_index, step):
